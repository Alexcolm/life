...
interface ScheduleFormProps {
  date: string
  onSubmit: (input: { date: string; time: string; activity: string; description: string }) => Promise<void>
}
...
export function ScheduleForm({ date, onSubmit }: ScheduleFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    try {
      await onSubmit({ date, time, activity: activity.trim(), description: description.trim() })
      setTime('')
      setActivity('')
      setDescription('')
    } finally {
